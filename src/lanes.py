...
class Lane:
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      ...
      (int(1*width),height-1), # Bottom-right corner
    ...
    self.YM_PER_PIX = 7.0 / 400 
    self.XM_PER_PIX = 3.7 / 255 		
  ...
  def get_line_markings(self, frame=None):
    ...
    sxbinary = edge.mag_thresh(sxbinary, sobel_kernel=3, thresh=(0, 255))
    s_channel = hls[:, :, 2] 
    _, s_binary = edge.threshold(s_channel, (25, 255))
  ...
  def histogram_peak(self):
    ...
    leftx_base = np.argmax(np.gradient(self.histogram[:midpoint]))
    rightx_base = np.argmin(np.gradient(self.histogram[midpoint:])) + midpoint
  ...
  def perspective_transform(self, frame=None, plot=False):
    ...
    if plot == True:
      ...
      warped_plot = cv2.polylines(warped_copy, np.int32([
                    self.desired_roi_points]), True, (147,100,255), 3)
